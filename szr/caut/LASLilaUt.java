...
import szr.card.*;
/**
 * Automata lila utasító állapot
 * Creation date: (2002.03.12. 23:00:23)
 */
public class LASLilaUt extends LAStateCJ {
	private static java.util.ResourceBundle resInfoTexts = java.util.ResourceBundle.getBundle("szr.shared.InfoTexts");  //$NON-NLS-1$
	
public LASLilaUt(Lila iC, Jelzes iJ) {
	super(iC, iJ);
}
...
public String toString() {
	return "((LASLilaUt[" + c.toString() + ", " + j.toString() + "]))";//$NON-NLS-3$//$NON-NLS-2$//$NON-NLS-1$
}
}
